...
abstract class DBRelationalAdapter<C extends Object> extends DBAdapter<C> {
  ...
  getAdapterInstantiator<C extends Object, A extends DBRelationalAdapter<C>>({
    String? name,
    Type? type,
  ...
  getAdapterInstantiatorsFromConfig<
    C extends Object,
    A extends DBRelationalAdapter<C>
  ...
  @override
  DBRelationalRepositoryAdapter<O>? createRepositoryAdapter<O>(
    String name, {
    String? tableName,
    Type? type,
  }) =>
...
class DBRelationalRepositoryAdapter<O> extends DBRepositoryAdapter<O> {
  ...
  DBRelationalRepositoryAdapter(
    DBRelationalAdapter super.databaseAdapter,
    super.name, {
    super.tableName,
    super.type,
  });
...
class DBRelationalEntityRepository<O extends Object>
  ...
  DBRelationalEntityRepository(
    DBRelationalAdapter super.adapter,
    super.name,
    super.entityHandler, {
    DBRelationalRepositoryAdapter<O>? super.repositoryAdapter,
    super.type,
  });
  ...
  FutureOr<List<O>> select(
    ...
    try {
      return repositoryAdapter.doSelect(
        op,
        matcher,
        parameters: parameters,
        positionalParameters: positionalParameters,
      ...
      () {
        ...
        if (changedFields != null) {
          if (changedFields.isEmpty) {
            return _ensureRelationshipsStored(
              o,
              op.transaction,
              changedFields,
            ).resolveWith(() {
  ...
  FutureOr<bool> _ensureRelationshipsStored(
    O o,
    Transaction? transaction, [
  ...
  FutureOr<Iterable<dynamic>> selectRelationship<E>(
    ...
    var cachedRelationship = _getCachedEntityRelationship(
      oId,
      field,
      fieldType,
      transaction,
    ...
    try {
      return repositoryAdapter.doSelectRelationship(
        op,
        field,
        oId,
        valuesTableName,
      ...
      var message =
          'selectRelationship[valuesTableName]> '
          'o: $o ; '
          'oId: $oId ; '
          'field: $field ; '
          'fieldType: $fieldType ; '
  ...
  Map<dynamic, List<dynamic>>? _getCachedEntitiesRelationships<E>(
    ...
    var cachedEntities = transaction.getCachedEntitiesByIDs(
      ids,
      type: type,
  ...
  FutureOr<Map<dynamic, Iterable<dynamic>>> selectRelationships<E>(
    ...
    var cachedRelationships = _getCachedEntitiesRelationships(
      oIds,
      field,
      fieldType,
      transaction,
    ...
    try {
      return repositoryAdapter.doSelectRelationships(
        op,
        field,
        oIds,
        valuesTableName,
        (sel) {
          var relationships = sel
              .groupListsBy((e) => e['source_id']!)
              .map(
                (id, l) => MapEntry(id, l.map((m) => m['target_id']).toList()),
              );

          if (cachedRelationships != null && cachedRelationships.isNotEmpty) {
            relationships = Map<dynamic, List<dynamic>>.fromEntries(
              oIdsOrig.map(
                (id) => MapEntry<dynamic, List<dynamic>>(
                  id,
                  relationships[id] ?? cachedRelationships[id] ?? [],
                ),
              ),
            );
          }
  ...
  String _resolveTableName(Type type) {
    var repository = repositoryAdapter.databaseAdapter
        .getRepositoryAdapterByType(type);
    if (repository == null) {
  ...
  EntityHandler<E> _resolveEntityHandler<E>(Type type) {
    var entityRepository = entityHandler.getEntityRepositoryByType(
      type,
...
abstract class DBRelationalEntityRepositoryProvider<
  A extends DBRelationalAdapter
>
    extends DBEntityRepositoryProvider {
  @override
